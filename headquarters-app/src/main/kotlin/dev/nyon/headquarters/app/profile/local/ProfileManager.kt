...
suspend fun createProfile(profile: LocalProfile) {
    ...
    if (testModProfile.loader == Loader.Fabric) {
        ...
        (loaderVersion.launcherMeta as LauncherMeta).libraries.common.forEach { artifact ->
            ...
            ktorClient.downloadFile(Url(url), librariesDir.resolve(fileName))
        }
    }

    modrinthConnector.getVersions(testModProfile.mods.map { it.versionID }).let {
        if (it == null) return@let
        profileDir.resolve("mods/").createDirectories().downloadProjects(it)
    }

    testModProfile.configs.forEach {
        withContext(Dispatchers.IO) {
            profileDir.resolve("config/${if (it.directory != "") "${it.directory}/${it.name}" else it.name}")
                .createDirectories().writeBytes(it.content)
        }
    }
}
